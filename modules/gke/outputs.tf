...
output "network_policy_enabled" {
  ...
  value       = coalescelist(lookup(coalescelist(google_container_cluster.main.addons_config, [{}])[0], "network_policy_config", [{}]), [{ disabled = false }])[0].disabled
}

output "http_load_balancing_enabled" {
  description = "Whether http load balancing enabled"
  value       = coalescelist(lookup(coalescelist(google_container_cluster.main.addons_config, [{}])[0], "http_load_balancing", [{}]), [{ disabled = false }])[0].disabled
}

output "horizontal_pod_autoscaling_enabled" {
  description = "Whether horizontal pod autoscaling enabled"
  value       = coalescelist(lookup(coalescelist(google_container_cluster.main.addons_config, [{}])[0], "horizontal_pod_autoscaling", [{}]), [{ disabled = false }])[0].disabled
}
